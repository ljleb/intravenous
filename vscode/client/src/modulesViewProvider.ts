...
const sourceCard=source=>{const card=document.createElement('div');card.className='card';const head=document.createElement('div');head.className='rowhead';const title=document.createElement('div');title.className='title';title.textContent=source.sourceId;const kind=document.createElement('div');kind.className='subtle';kind.textContent=source.projectLocal?'Local':'Shared';head.append(title,kind);const meta=document.createElement('div');meta.className='meta';meta.textContent=source.moduleRoot;const actions=document.createElement('div');actions.className='actions';for(const moduleId of source.moduleIds){actions.append(button('add','Instantiate '+moduleId,'primary',()=>action('instantiate',{moduleId})))}if(!source.moduleIds.length){const pending=document.createElement('span');pending.className='subtle';pending.textContent='No published modules';actions.append(pending)}if(source.nodeTypeIds.length){const nodes=document.createElement('span');nodes.className='subtle';nodes.textContent='Node types: '+source.nodeTypeIds.join(', ');actions.append(nodes)}actions.append(button('code','Open module source','',()=>action('reveal',{moduleRoot:source.moduleRoot})));card.append(head,meta,actions);return card};

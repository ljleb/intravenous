...
export type ModulesControlMessage =
    | { type: "createPackage"; name: string }
    | { type: "instantiate"; moduleId: string }
    | { type: "open"; instanceId: string; packageRoot: string }
    | { type: "rename"; instanceId: string; displayName: string }
    | { type: "duplicate"; moduleId: string }
    | { type: "delete"; instanceId: string }
    | { type: "reveal"; packageRoot: string }
...
export class ModulesViewProvider {
    ...
    private isControlMessage(value: unknown): value is ModulesControlMessage {
        ...
        switch (message.type) {
        case "createPackage": return typeof message.name === "string";
        case "instantiate":
        case "duplicate": return typeof message.moduleId === "string";
        case "reveal": return typeof message.packageRoot === "string";
        case "open": return typeof message.instanceId === "string" && typeof message.packageRoot === "string";
...
const packageCard=packageInfo=>{const card=document.createElement('div');card.className='card';const head=document.createElement('div');head.className='rowhead';const title=document.createElement('div');title.className='title';title.textContent=packageInfo.packageId;const kind=document.createElement('div');kind.className='subtle';kind.textContent=packageInfo.projectLocal?'Local':'Shared';head.append(title,kind);const meta=document.createElement('div');meta.className='meta';meta.textContent=packageInfo.packageRoot;const actions=document.createElement('div');actions.className='actions';for(const moduleId of packageInfo.moduleIds){actions.append(button('add','Instantiate '+moduleId,'primary',()=>action('instantiate',{moduleId})))}if(!packageInfo.moduleIds.length){const pending=document.createElement('span');pending.className='subtle';pending.textContent='No published modules';actions.append(pending)}if(packageInfo.nodeTypeIds.length){const nodes=document.createElement('span');nodes.className='subtle';nodes.textContent='Node types: '+packageInfo.nodeTypeIds.join(', ');actions.append(nodes)}actions.append(button('code','Open module source','',()=>action('reveal',{packageRoot:packageInfo.packageRoot})));card.append(head,meta,actions);return card};

...
Options:
  --module PATH        Source directory or iv_source.json to snapshot.
...
if (( simple_sine_modules )); then
    for manifest in "$repo_root"/projects/simple_sine/modules/*/iv_source.json; do
        [[ -f "$manifest" ]] || continue
...
for module_index in "${!module_paths[@]}"; do
    ...
    if [[ "${module_name##*/}" == "iv_source.json" ]]; then

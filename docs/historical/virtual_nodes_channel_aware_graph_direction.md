...
| **Channel conversion** | A planned edge operation that converts between compatible channel types/layouts while preserving each port's declared representation. |
...
only to attach the same quoted-name span to the correct public-output index;
...
- Compatible types with different representations use the channel conversion
  ...
  connect to a mid/side stereo port without either port performing manual
  conversion.
...
but do not appear as configured nodes, sidepanel entries, or lane ports.
...
```text
{ module instance, virtual node identity, port kind, virtual-port index }
```

The node-bundle state is addressed by the stable index of that bundle port
within the virtual-port mapping. This index is not a build-local
`NodeBundlePortId`, concrete-node ID, or tile index. It preserves the
...
output ports for every channel member. A lane
...
- **Tiled node insertion, values, lowering, and old-model removal.** Add an
  ...
  ports lower to direct mono edges, and native/tiled boundaries lower

...
> Computed tock outputs publish exact finite `Coverage`. Forward
...
> pointwise tick node's temporal propagation may be generated by GraphJit.
...
Port data (sample/event), input access, output production, and output retention
...
| data | sample, event | value/event representation and bounds |
...
represented by one `Coverage` bit per sample.
...
demand. Semantic changes to any published retained source start downstream exact
...
propagation remains exact in `Coverage` region space.
...
tick pointwise nodes use compiler-generated coverage propagation in the same
...
coverage callback: GraphJit generates the exact same-position temporal mapping
...
generate identity-time forward/reverse propagation for the replayable node's
...
pointwise tick nodes use compiler-generated temporal propagation.
...
8. Exact `Coverage` and forward changes are independent of the storage
   page grid. Reverse demand is value-blind; replayable pointwise tick propagation
   is generated from static dependencies.

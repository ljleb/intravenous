...
- **No audio-thread setup.** Root execution performs no heap allocation, lazy
  ...
  Per-root sequence resets, SCC cursors, and temporal warmup countdowns are
  execution semantics, not deferred graph construction. Any bounded invocation-
...
- **Samples:** ordinary internal realtime transport is complete across direct and
  ...
  transport mode: root/external boundary binding and realtime/compiled mixed or
  compiled-only access still belong to points 13 and 15.
- **Events, feed-forward:** internal realtime direct/transient sequences, block
  adaptation, non-expanding conversion, fanout, compact retained carry, persistent
  rings, and retained converted fanout are implemented for one logical producer
  stream. Merge/fan-in from multiple independent event producers is analyzed and
  statically sized but remains capability-gated in physical lowering.
- **Events, cyclic:** `detach()` currently requires one semantic source, exact event
  ...
  connection work.
- **Both kinds:** external project boundaries and compiled-access directions remain
  explicit lowering gates. Those are separate execution/boundary capabilities, not
  deficiencies in the internal realtime feed-forward planners.
...
cursors are stack/SSA state for one root call; and sample alignment `warmup_remaining`
is migrated temporal state that counts down as samples become valid. None of those
are substitutes for graph initialization.
...
10. **Event-port realization refactor and simple event flow.** **Landed for direct, transient block adaptation, conversion, and feed-forward fanout.**
    ...
    share one derived representation/materialization. This currently covers one
    logical producer stream per event group; multi-producer event fan-in remains
    capability-gated even though analysis already records and sizes merged sources.
...
12. **SCC/feedback execution.** **Sample feedback and the first event-feedback
    ...
    connection gates are external project boundaries and compiled-access directions.
    ...
    root-call sizes, and generation migration. Ordinary feed-forward event fan-in
    from multiple independent producers, cyclic event conversion, history/latency
    retention, edges/fanout leaving a cyclic region, and edges spanning distinct
    cyclic regions remain capability-gated.

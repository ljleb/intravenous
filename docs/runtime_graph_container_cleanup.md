...
- dense handle/ordinal domains -> offset vectors/bitmaps;
...
The IDs are composed of node-bundle handles, port ordinals, and for samples,
channel ordinals. All three dimensions are known from the node bundles. Dense
bitmaps or offset-indexed byte vectors are therefore possible and likely more
...
ordinals are bounded by the configured node bundles, a dense index can be
...
input slots. This is not specifically a constexpr workaround, but it is worth

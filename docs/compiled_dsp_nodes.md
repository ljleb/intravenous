...
There is no small fixed limit on how many distinct node instances may contribute
requested compiled outputs to one logical query. In the extreme, every
requestable compiled-output node in the project may participate.

The obvious example is the UI: all currently visible compiled lanes should be submitted as one query.

The project root itself does **not** gain synthetic compiled output ports or a
project-wide `access_block()`. The optimized project root remains a zero-input,
zero-output node. `CompiledGraph` instead carries immutable metadata mapping
internal `(node, compiled output port)` endpoints to specialized compiled-access
executors generated by whole-project lowering.
...
### Whole-project lowering specializes the static part

The topology of compiled-port dependencies does not change between queries for
one compiled graph generation. `GraphJit` should therefore lower most of the
planning structure ahead of time rather than rediscovering it through generic
graph traversal on every request.

Lowering may partition compiled-port edges into independent connected
components and precompute for each component:

* requestable sink/output ordinals;
* reverse dependency order for propagation;
* forward topological order for evaluation;
* fanout/convergence structure;
* constant node/port/state offsets;
* which propagation operations are trivial; and
* bounded reusable request/workspace storage where useful.

At runtime, requested internal outputs are grouped by component. All requested
sinks in one component are seeded before reverse propagation begins. The
generated executor then walks the precomputed reverse order, unions requests at
converging ports, and finally walks the precomputed forward order to evaluate
the complete demand. Disconnected components may execute independently because
they cannot share upstream compiled work.

The dynamic information is primarily the request-set contents: sparse sample
grids, dense ranges, and event intervals. The runtime should not need to rebuild
adjacency lists or topologically sort the graph for each query.
...
## 15. Node lifecycle, `NodeLayout`, and `CompiledState`
...
All four need appropriate support for `CompiledState` as well as normal realtime
`State`. This support belongs in the canonical `NodeLayout`/`NodeStorage`
...
* pack state and compiler-owned regions in one arena;
* choose declaration order for cache locality;
* colocate `State`, `CompiledState`, history, and reusable workspaces where useful;
* inline or eliminate storage that optimization proves unnecessary;
* omit absent states; and
* share immutable precomputed objects where valid.

Request-sized caller inputs/results may remain outside `NodeStorage` when their
size is not bounded at graph-compilation time. Persistent state and bounded
reusable project workspaces should not require a second project storage object.

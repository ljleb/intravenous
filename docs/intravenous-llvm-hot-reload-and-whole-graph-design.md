...
Until the whole-project kernel replaces the compatibility executor, `ModuleLoader` optimizes only the in-memory package copy that it hands to the shared ORC JIT. It removes Clang's O0 optimization barrier only from the transitive code reachable from node `tick_block`/`skip_block` callbacks, runs the default O3 module pipeline, and uses aggressive native code generation. Graph-construction callbacks remain O0 because package configuration is allowed to throw and catch diagnostics across the JIT boundary. This does not rewrite the package artifact and does not reintroduce a per-package native-library boundary.

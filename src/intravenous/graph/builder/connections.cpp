...
std::unordered_map<std::string, std::unordered_set<std::string>> types_by_virtual_id_for(
    GraphBuilderTopology const& topology)
{
    std::unordered_map<std::string, std::unordered_set<std::string>> types_by_virtual_id;
    for (size_t node_i = 0; node_i < topology.node_count(); ++node_i) {
        auto const& node = topology.node(node_i);
        if (node.materialization.is_placeholder() || node.vacant_input_ownership.virtual_node_id.empty()) {
            continue;
        }
        types_by_virtual_id[node.vacant_input_ownership.virtual_node_id].insert(node.type_identity.value);
    }
    return types_by_virtual_id;
}

std::string final_virtual_node_id_for(
    std::unordered_map<std::string, std::unordered_set<std::string>> const& types_by_virtual_id,
    BuilderNode const& node)
{
    std::string final_virtual_node_id = node.vacant_input_ownership.virtual_node_id;
    if (auto it = types_by_virtual_id.find(node.vacant_input_ownership.virtual_node_id);
        it != types_by_virtual_id.end() && it->second.size() > 1) {
        final_virtual_node_id += "#type:" + details::stable_identity_suffix(node.type_identity.value);
    }
    return final_virtual_node_id;
}
} // namespace
...
GraphBuilderVacantInputs GraphBuilderConnections::collect_vacant_inputs(GraphBuilderTopology const& topology) const
{
    ...
    std::unordered_map<std::string, size_t> next_member_ordinal_by_virtual_id;
    for (size_t node_i = 0; node_i < original_node_count; ++node_i) {
        auto const& node = topology.node(node_i);
        if (node.materialization.is_placeholder()) {
            continue;
        }
        std::string const virtual_node_id = node.vacant_input_ownership.virtual_node_id;
        if (virtual_node_id.empty()) {
            continue;
        }
        std::string final_virtual_node_id = virtual_node_id;
        if (auto it = types_by_virtual_id.find(virtual_node_id);
            it != types_by_virtual_id.end() && it->second.size() > 1) {
            final_virtual_node_id += "#type:" + details::stable_identity_suffix(node.type_identity.value);
        }
        size_t const member_ordinal = next_member_ordinal_by_virtual_id[final_virtual_node_id]++;
        for (size_t input_i = 0; input_i < node.inputs().size(); ++input_i) {
            ...
            result.sample.push_back(GraphBuilderVacantSampleInput{
                .target = target_port,
                .virtual_node_id = final_virtual_node_id,
    ...
    GraphBuilderTopology const& topology) const
{
    ...
    size_t const original_node_count = topology.node_count();
    auto const types_by_virtual_id = types_by_virtual_id_for(topology);
    ...
    for (size_t node_i = 0; node_i < original_node_count; ++node_i) {
        ...
        if (node.vacant_input_ownership.virtual_node_id.empty()) {
            continue;
        }
        std::string const final_virtual_node_id = final_virtual_node_id_for(types_by_virtual_id, node);
        size_t const member_ordinal = next_member_ordinal_by_virtual_id[final_virtual_node_id]++;
        for (size_t input_i = 0; input_i < node.inputs().size(); ++input_i) {
            PortId const target_port{ node_i, input_i };
    ...
    GraphBuilderTopology const& topology) const
{
    ...
    struct FamilyKeyHash {
        ...
        {
            size_t hash = std::hash<std::string>{}(key.virtual_node_id);
    ...
    for (size_t node_i = 0; node_i < topology.node_count(); ++node_i) {
        ...
        auto const final_virtual_node_id = final_virtual_node_id_for(types_by_virtual_id, node);
        ...
        for (size_t input_i = 0; input_i < node.inputs().size(); ++input_i) {
            ...
            if (!families_by_key.contains(key)) {
                ...
                families_by_key.emplace(key, PendingFamily{
                    ...
                    .channels = std::vector<GraphBuilderVirtualSampleInputChannel>(
            ...
            if (family.channel_type != channel_type) {
                details::error(
                    "conflicting channel family types for virtual node " + final_virtual_node_id);
            }
            if (channel_ordinal >= family.channels.size()) {
                details::error("channel ordinal out of bounds while collecting virtual sample inputs");
            }
            auto& channel = family.channels[channel_ordinal];
            if (channel.target.has_value()) {
                details::error(
                    "duplicate channel contributor for virtual sample input family on " + final_virtual_node_id);
            }
            PortId const target_port{ node_i, input_i };
    ...
    std::unordered_map<std::string, std::vector<FamilyKey>> families_by_virtual_member;
    for (auto const& key : family_order) {
        families_by_virtual_member[key.virtual_node_id + "\x1f" + key.member_group_key].push_back(key);
    }
    std::unordered_map<std::string, size_t> member_ordinal_by_virtual_member;
    for (auto& [_, keys] : families_by_virtual_member) {
    ...
    for (auto const& key : family_order) {
        auto const member_key = key.virtual_node_id + "\x1f" + key.member_group_key;
        if (!seen_member_groups.emplace(member_key, true).second) {
            continue;
        }
        auto& keys = families_by_virtual_member.at(member_key);
        size_t const member_ordinal = member_ordinal_by_virtual_member[key.virtual_node_id]++;
    ...
    for (auto const& [key, member_ordinal] : ordered_keys) {
        ...
        result.families.push_back(GraphBuilderVirtualSampleInputFamily{
            .virtual_node_id = std::move(family.virtual_node_id),
            .member_ordinal = member_ordinal,
        ...
        for (size_t output_i = 0; output_i < node.outputs().size(); ++output_i) {
            ...
            if (!families_by_key.contains(key)) {
                ...
                families_by_key.emplace(key, PendingFamily{
                    ...
                    .config = config,
                    .channel_type = channel_type,
                    .channels = std::vector<GraphBuilderVirtualSampleOutputChannel>(
                        channel_count(channel_type)),
                });
            }
            ...
            if (channel_ordinal >= family.channels.size()) {
                details::error("channel ordinal out of bounds while collecting virtual sample outputs");
            }
            auto& channel = family.channels[channel_ordinal];
            if (channel.source.has_value()) {
                details::error(
                    "duplicate channel contributor for virtual sample output family on " + final_virtual_node_id);
            }
            PortId const source_port{ node_i, output_i };

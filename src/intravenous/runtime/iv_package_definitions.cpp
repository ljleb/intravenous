...
std::vector<IvPackageInfo> discover_iv_package_definitions(
    ...
    std::vector<std::filesystem::path> const& shared_roots)
{
    ...
    for (auto const& package : discover_iv_package_definitions(project_root, shared_roots)) {
        declarations.emplace_back(package.package_id, package.package_root);
    }
    return declarations;
}

IvPackageDefinitions::IvPackageDefinitions(std::filesystem::path project_root)
    : project_root_(std::filesystem::weakly_canonical(std::move(project_root)).lexically_normal())
{
}

IvPackageInfo& IvPackageDefinitions::ensure_package(
...
std::vector<IvPackageInfo> IvPackageDefinitions::list_packages() const
{
    std::vector<IvPackageInfo> packages;
    {
        std::scoped_lock lock(mutex_);
        packages.reserve(packages_by_id_.size());
        for (auto const &[_, package] : packages_by_id_) {
...
void IvPackageDefinitions::handle_package_declarations_changed(
    IvPackageDeclarationsChanged const &diff)
{
    std::scoped_lock lock(mutex_);
    for (auto const &declaration : diff.created) {
        (void)ensure_package(declaration.package_id, declaration.package_root);
    }
...
void IvPackageDefinitions::handle_package_definitions_changed(
    IvPackageDefinitionsChanged const &diff)
{
    std::scoped_lock lock(mutex_);
    auto erase_id = [&](std::string const &id) {
        for (auto &[_, package] : packages_by_id_) {
            std::erase(package.module_ids, id);
    ...
    for (auto &[package_id, package] : packages_by_id_) {
        ...
        package.publication_message = message == diff.publication_messages_by_package_id.end()
            ? std::string{}
            : message->second;
    }
}
    ...
    std::vector<IvPackageBuildStatus> const &statuses)
{
    ...
    for (auto const &status : statuses) {
        ...
        package.build_state = status.state;
        package.build_message = status.message;
    }
    for (auto &[package_id, package] : packages_by_id_) {
...
void IvPackageDefinitions::handle_socket_rpc_get_iv_package_definitions(
...
void IvPackageDefinitions::handle_socket_rpc_create_iv_package(
...
IvPackageInfo IvPackageDefinitions::create_project_package(std::string const& name) const
{

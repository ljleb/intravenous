...
void handle_delete_timeline_lane(
    DeleteTimelineLaneRequest const& request, SocketRpcAckResponseBuilder &builder)
{
    try {
        ProjectAckBuilder project_builder;
        IV_INVOKE_LINKER_EVENT(
            iv_runtime_project_delete_timeline_lane_requested_event,
            ProjectDeleteTimelineLaneRequest{.lane_id = request.lane_id}, project_builder);
        project_builder.build();
    } catch (std::exception const& error) {
        builder.fail(error.what());
    }
